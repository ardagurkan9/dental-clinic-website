          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div>
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="text-[10px] tracking-[0.28em] uppercase font-medium" style={{ color: "#B8922A" }}>{overline}</span>
              </div>

              <h2 className="font-cormorant text-4xl sm:text-5xl leading-tight mb-8">
                <span className="font-light italic" style={{ color: "#0F1E35" }}>{heading}</span>
              </h2>

              {paragraphs.map((p, i) => (
                <p key={i} className="leading-relaxed mb-4 text-[15px] last:mb-10" style={{ color: "#8A7E70" }}>{p}</p>
              ))}

              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-12">
                {benefits.map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm" style={{ color: "#8A7E70" }}>
                    <span className="mt-0.5 shrink-0 text-base leading-none" style={{ color: "#B8922A" }}>◈</span>
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="relative">
              ...
              <div className="aspect-[4/3] flex items-center justify-center relative overflow-hidden"
                ...
                <div className="text-center">
                  <svg className="w-14 h-14 mx-auto mb-4" fill="none" stroke="currentColor" strokeWidth={0.8} viewBox="0 0 24 24"
                    style={{ color: "rgba(184,146,42,0.35)" }}>
                    <rect x="3" y="3" width="18" height="18" rx="0" />
                    <circle cx="8.5" cy="8.5" r="1.5" />
                    <polyline points="21 15 16 10 5 21" />
                  </svg>
                  <p className="text-[11px] tracking-[0.22em] uppercase" style={{ color: "rgba(184,146,42,0.5)" }}>Tedavi Görseli</p>
                </div>
              </div>
              ...
              <div className="absolute -bottom-6 -right-6 px-8 py-5 z-20 text-white"
                ...
                <p className="font-cormorant text-4xl font-bold leading-none">
